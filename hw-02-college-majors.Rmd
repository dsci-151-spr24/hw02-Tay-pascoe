...
The data can be found in the **fivethirtyeight** package, and it's called `college_recent_?grads`.
...
```
***The major with the lowest unemployment rate is Petrolium Engineering***
...
***The major with the lowest unemployment rate is Mathematics and Computer Science ***
...
***In descending order its psychology, biology, and Business managment and administractions***
```{r}
college_recent_grads %>%
  arrange(desc(women)) %>%
...
***I chose the binwidth of 1000 because it better shows the data individually instead of a 5000 which grouped the data to much to be able to read as wel.***
...
***The median would be the best summary statistic to describe the distribution. I'm not sure how to answer the second part of this question.***
...
```{r}
ggplot(data = college_recent_grads, mapping = aes(x = median)) +
  geom_histogram(binwidth = 1000) + facet_wrap(~major_category)
...
***the least popular major is agriculture and natural resources***
...
```{r, eval = TRUE}
college_recent_grad_stem_categories <- c("Biology & Life Science",
                     "Computers & Mathematics",
                     "Engineering",
                     "Physical Sciences") 
ggplot(college_recent_grads, aes (x = median, y = women, color = major_type)) +
geom_point()
  labs(
    x = "Median Income", 
    y = "Women in major",
    title = "Median income vs women in major"
  )
```
***this data shows the relationship between the women who are in a stem major or are not, and  their median income based on that. It looks like there are more women not in stem but the few that are, are getting paid a bit more.***

## Further exploration

10. Ask a question of interest to you, and answer it using summary statistic(s) and/or visualization(s).

***Question: Which major categories make the least money?***

```{r}

  
ggplot(college_recent_grads, aes(x = median, y = major_category, color = low_wage_jobs)) +
  geom_col(alpha=1)
```
***It looks like interdisciplinary and communications and journalism are the lowest paying major categories with engineering work being the highest***
